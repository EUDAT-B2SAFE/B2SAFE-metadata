...
class MetsManifestComparator():
    ...
    def contentsFromStructMap(self, structMap, logicalCollections, defaultDivs):
        for smap in structMap:
            mainDiv = smap.div
            for innerDiv in mainDiv.content() :
                if innerDiv.TYPE == "entityRelation":
                    logicalCollections[innerDiv.LABEL] = innerDiv
                else:
                    innerDiv_id = self.createIdFrom(innerDiv)
                    if innerDiv_id != "":
                        defaultDivs[innerDiv_id] = innerDiv
    ...
    def createMapFromDiv(self, div):
        ...
        for entry in div.content():
            if type(entry) == divType:
                entry_id = self.createIdFrom(entry)
                if entry_id != "":
                    mapFromDiv[entry_id] = entry
            else:
